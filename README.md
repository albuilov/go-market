...
Runtime-код находится в одном корневом Go-модуле. Инструменты генерации
изолированы в модуле `tools`, поэтому их зависимости не попадают в сервисы.
Файл `go.work` проекту не нужен: команды из каталогов сервисов также работают
с `GOWORK=off`.
...
Проверить Go-код, protobuf-контракты и frontend:

```bash
make check
```

Отдельно доступны команды `make vet`, `make frontend-check`,
`make proto-lint` и `make proto-breaking`.
...
Сгенерированные файлы в `gen` хранятся в Git. После изменения `.proto` нужно
выполнить `make proto` и закоммитить контракт вместе с результатом генерации.

## Документация

- [Архитектура](docs/architecture.md);
- [Добавление нового сервиса](docs/development/adding-service.md);
- [Соглашения публичного API](docs/api-conventions.md);
- [Границы доверия и авторизация](docs/security.md);
- [Health checks и завершение работы](docs/operations.md).
